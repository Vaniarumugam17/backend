...
exports.addToPlaylist = (req, res, next) => {
  ...
  Product.findOne({ _id: audioId, userId: req.user._id })
    .then((product) => {
...
exports.removeFromPlaylist = (req, res, next) => {
  ...
  req.user
    ...
    .catch((err) => {
      if (
        err.message ===
        "No such music in your playlist. Please check once again."
      ) {
        return next(new Error(err.message));
      }
      next(new Error("Error! Please try again."));
  ...
  Product.find({ userId: req.user._id })
    .then((products) => {
      ...
      filteredProds.sort((a, b) => {
        ...
        if (a[1] == b[1]) {
          if (
            searchAlgo.minChanges(a[0].title, filter) ==
            searchAlgo.minChanges(b[0].title, filter)
          ) {
            return 0;
          } else {
            return searchAlgo.minChanges(a[0].title, filter) <
              searchAlgo.minChanges(b[0].title, filter)
              ? -1
              : 1;
          }
