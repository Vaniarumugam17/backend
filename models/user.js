...
userSchema.methods.addToPlaylist = function (product) {
  ...
  const x = this.playList.find(
    (item) => item.audioId.toString() === product._id.toString()
  );
  if (x) {
...
userSchema.methods.removeFromPlaylist = function (audioId) {
  const productInd = this.playList.findIndex(
    (item) => item.audioId.toString() === audioId
  );
  if (productInd === -1) {
...
userSchema.methods.clearPlaylist = function (arr) {
  ...
  for (let item of this.playList) {
    if (
      arrInd < arr.length &&
      item._id.toString() === arr[arrInd]._id.toString()
    ) {
      arrInd++;
      continue;
    }
