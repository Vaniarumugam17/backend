...
router.post(
  ...
  [
        ...
        return User.findOne({ email: value }).then((user) => {
          if (user) {
            return Promise.reject(
              "E-mail already exists! Please use a different one!"
            );
          }
        });
      }),
    body(
      "password",
      "Please enter a password with only numbers/text and atleast 5 chars long !"
    )
      .isLength({ min: 5, max: 30 })
      .isAlphanumeric()
      .trim(),
    ...
    body("email")
      ...
      .custom((value, { req }) => {
        return User.findOne({ email: value }).then((user) => {
          if (!user) {
            return Promise.reject(
              "User not found. Enter an existing email id!"
            );
          }
        });
      }),
